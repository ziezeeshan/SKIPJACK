...
# example with user input for plaintext
try:
    input_string = input("Enter the 8-character string to encrypt: ")
    if len(input_string) != 8:
        raise ValueError
except ValueError:
    print("Invalid input. Please enter an 8-character string.")
    exit(1)

KEY = [0x00, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11]
sj = SkipJack()
CT = sj.encrypt_string(input_string, KEY)

if CT is not None:
    print("Input string:", input_string)
    print("Cipher text:", hex(CT))  # Displaying ciphertext in hexadecimal format
